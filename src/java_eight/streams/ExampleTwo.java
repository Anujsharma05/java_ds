...
public class ExampleTwo {
	...
	public static void main(String[] args) {
		...
		employeeList.add(new Employee(199, "Wang Liu", 24, "Female", "Sales And Marketing", 2016, 11500.0));
		...
		employeeList.add(new Employee(279, "Anuj Chettiar", 34, "Male", "Product Development", 2012, 35700.0));

		HashSet<String> allItems = new HashSet<>();
		Predicate<Employee> pred = emp -> !allItems.add(emp.getName());

		Set<String> duplicates = employeeList.stream().filter(pred).map(Employee::getName).collect(Collectors.toSet());

		System.err.println(duplicates);
		List<Integer> ages = Arrays.asList(1, 2, 3, 4, 5);
		int factorial = ages.stream().reduce(1, (prod, el) -> prod * el);
